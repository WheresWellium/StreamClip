...
| CI arm64 on Latest | ✅ `STREAMCLIP_MAC_SINGLE_ARCH=arm64` via `desktop-release.yml` → `qClip-mac-arm64.dmg` on **v1.0.0-beta.25** |
...
gh release upload v1.0.0-beta.25 apps/desktop/release/qClip-mac-universal.dmg --clobber
```

CI (Apple Silicon only, no Rosetta):

```bash
gh workflow run desktop-release.yml -f version=1.0.0-beta.25 -f skip_windows=true
```
