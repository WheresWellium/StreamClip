...
import {
  updateMemoryPreferencesAction,
  updatePrivacyOptInAction,
  wipeUserPreferencesAction,
} from "@/lib/api/actions/support";
...
export function PrivacyPanel({ isAuthenticated, initialOptIn, initialPreferences }: Props) {
  const { push: toast } = useToastSafe();
  const [optIn, setOptIn] = React.useState(initialOptIn);
  const [memoryEnabled, setMemoryEnabled] = React.useState(
    initialPreferences?.memory_enabled ?? false,
  );
  const [pending, setPending] = React.useState(false);
  const [memoryPending, setMemoryPending] = React.useState(false);
  const [wipePending, setWipePending] = React.useState(false);
  ...
  React.useEffect(() => {
    setMemoryEnabled(initialPreferences?.memory_enabled ?? false);
  }, [initialPreferences?.memory_enabled]);
  ...
  async function handleMemoryToggle(next: boolean) {
    ...
    const result = await updateMemoryPreferencesAction(next);
    setMemoryPending(false);
    if (!result.ok) {
      toast("Could not save", result.message ?? "Try again.");
      return;
    }
    setMemoryEnabled(result.memoryEnabled ?? next);
    toast(
      next ? "Personalization enabled" : "Personalization disabled",
      next
        ? "Title and transcription hints can use your saved preferences."
        : "Preferences are ignored until you turn this back on.",
    );
  }

  async function handleWipe() {
    setWipePending(true);
    const result = await wipeUserPreferencesAction();
    setWipePending(false);
    if (!result.ok) {
      toast("Could not wipe", result.message ?? "Try again.");
      return;
    }
    setMemoryEnabled(false);
    toast("Preferences wiped", "Your saved vocabulary and title hints were cleared.");
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Optional on-device preferences for titles and transcription — never sent to
        ...
        <CardContent className="space-y-4">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              ...
              disabled={!isAuthenticated || memoryPending}
              onChange={(e) => handleMemoryToggle(e.target.checked)}
              className="mt-0.5 accent-sky-500"
            />
            <span className="text-sm">
              <span className="font-medium flex items-center gap-2">
                Use my saved preferences for titles and transcription
          ...
          {!isAuthenticated && (
            <p className="text-xs text-muted-foreground">Sign in to manage preferences.</p>
          )}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button
              ...
              variant="outline"
              size="sm"
              disabled={!isAuthenticated || wipePending}
              onClick={() => void handleWipe()}
            >
              {wipePending ? "Wiping…" : "Wipe my preferences"}
            </Button>
            <span className="text-xs text-muted-foreground">
              Clears vocabulary, tags, and title hints stored on your account.
            </span>
          </div>
        </CardContent>
      </Card>
