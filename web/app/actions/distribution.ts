...
export async function startOAuthAction(platform: string): Promise<void> {
  ...
  if (!session.ok) {
    ...
    redirect("/distribution?error=pro_required");
  }
  try {
    ...
    redirect(`/distribution?error=${encodeURIComponent(code)}`);
  }
}
