...
1. **Whisper size:** `small` (recommended, fastest) or `medium` (better transcripts, ~3x the download)?
2. **Docker path:** retire it from tester-facing docs entirely, or keep it as a clearly-labeled advanced page? Recommendation: keep, but unlinked from the main flow.
3. **Re-activation on upgrade:** acceptable for the 8 current testers, or should Phase A also accept the old placeholder-signed entitlements for one release to avoid any re-activation?
