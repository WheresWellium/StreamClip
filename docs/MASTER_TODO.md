...
| 4.22 | ~~**Feedback black hole (F13)**~~ ✅ 2026-08-03 — packaged desktop sets `OPS_WEBHOOK_URL` → `https://streamclip-henna.vercel.app/api/support-ingest`; henna Vercel has `SMTP_*` + `BUG_REPORT_TO`; GET health + POST email delivery verified; packaged sidecar smoke `ops_notification=queued`. Detail: [`DESKTOP_FAILURE_TAXONOMY.md`](DESKTOP_FAILURE_TAXONOMY.md) F13 | ✅ | M |
